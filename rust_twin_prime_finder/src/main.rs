...
fn ranking_label(soft_to: Option<usize>, hard_cutoff: usize) -> String {
...
fn find_twin_candidates(
    ...
    let max_prime_needed = max(hard_cutoff, soft_to.unwrap_or(hard_cutoff));
    ...
    if let Some(soft_limit) = soft_to {
        if soft_limit > hard_cutoff {
    ...
    FinderResult {
        ...
        ranking: ranking_label(soft_to, hard_cutoff),
        candidates,
    }
}
...
fn result_to_json(result: &FinderResult) -> String {
    ...
    out.push_str(&format!("  \"score_z\": null,\n"));
...
fn write_result_csv(result: &FinderResult, path: &str) -> io::Result<()> {
    ...
    writeln!(file, "rank,n,left,right,mode,hard_cutoff,ranking,extra_blocker,rank_value")?;
    for c in &result.candidates {
        ...
        writeln!(
            file,
            "{},{},{},{},{},{},{},{},{}",
            c.rank, c.n, c.n, c.pair_right, c.mode, c.hard_cutoff, result.ranking, extra, rank_value
        )?;
    }
    Ok(())
}
...
fn write_benchmark_csv(rows: &[BenchmarkRow], path: &str) -> io::Result<()> {
    ...
    writeln!(
        file,
        "limit_n,mode,hard_cutoff,soft_to,ranking,candidate_count,true_twins,total_twins,precision,recall,f1,elimination_rate"
    )?;
    for row in rows {
    ...
    if benchmark {
        ...
        for &limit in &benchmark_limits {
            for mode_name in &benchmark_modes {
                let result = find_twin_candidates(limit, mode_name, soft_to, top_k, cutoff);
        ...
        let result = find_twin_candidates(limit_n, &mode, soft_to, top_k, cutoff);
